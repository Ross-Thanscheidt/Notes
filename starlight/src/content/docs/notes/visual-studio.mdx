...
Install the [Customize Visual Studio Window Title](https://marketplace.visualstudio.com/items?itemName=mayerwin.RenameVisualStudioWindowTitle) extension to be able to customize the Window Title that shows up at the top of the Visual Studio windows and on the taskbar.  There are a number of special tags that can be used, including solution file parent directory name, git branch name, git repository name, and environment variable values.

* Install the extension in Visual Studio
* **Tools → Options → Environment → Visual Experience**
  * Check **Use compact menu bar** (requires restarting Visual Studio)
* **Tools → Options → Customize VS Window Title → Global rules → Patterns**
...
* **Tools → Options → Customize VS Window Title → Solution-specific overrides**
...
If the VS solution is not in a Git repository, the Windows title will be blank since `gitRepositoryName` is blank.
If you want a non-blank Windows title, then create a `<solution-name>.sln.rn.xml` file in the same directory as the `<solution-name>.sln` file with this content:
...
## Go to Next Error, Search Result, Reference
...
| `Ctrl`+`R`, `Ctrl`+`W`                             | View / Hide White Space |
...
| `Ctrl`+`K`, `Ctrl`+`D`                             | Format Document (Text Editor)<br />**Edit → Format Document** |
| `Ctrl`+`R`, `Ctrl`+`G`                             | Remove and Sort `using` statements in C#<br />right-click in Text Editor → **Remove and Sort Usings** |
| `Ctrl`+`K`, `Ctrl`+`E`                             | Format Document / Remove and Sort Usings / File Header Preferences<br />**Project → Code Cleanup → Run Code Cleanup (Profile 1) on Solution** |
| `Ctrl`+`R`, `T`                                    | Run Test for Current Unit Test Method |
| `Ctrl`+`R`, `Ctrl`+`T`                             | Debug Test for Current Unit Test Method |
| `Ctrl`+`E`, `T`                                    | Show Test Explorer |
| `Shift`+`Esc`                                      | Close Test Explorer |
...
These are my configuration tasks after installing or repairing Visual Studio:
...
* Install [Visual Studio Extensions](#visual-studio-extensions-1)
...
These are my custom keyboard shortcuts in Visual Studio:

* **Tools → Options → All Settings → Environment → Keyboard**
...
* [Bookmark Studio [Mads Kristensen]](https://marketplace.visualstudio.com/items?itemName=MadsKristensen.BookmarkStudio)

  <details>
      <summary>Keyboard Shortcuts</summary>

  | Extension Shortcut                          | Visual Studio Shortcut                     | Description |
  | :---                                        | :---                                       | :--- |
  | `Alt`+`Shift`+`B`                           |                                            | Open Bookmark Manager |
  | `Alt`+`Shift`+`1` to<br />`Alt`+`Shift`+`9` |                                            | Jump to bookmark 1 to 9 |
  | `Alt`+`Shift`+`N`                           | `Ctrl`+`K`, `Ctrl`+`N`                     | Next bookmark |
  |                                             | `Ctrl`+`Shift`+`K`<br />`Ctrl`+`Shift`+`N` | Next bookmark in document |
  | `Alt`+`Shift`+`H`                           | `Ctrl`+`K`, `Ctrl`+`P`                     | Previous bookmark |
  |                                             | `Ctrl`+`Shift`+`K`<br />`Ctrl`+`Shift`+`P` | Previous bookmark in document |
  | `Alt`+`Shift`+`<space>`                     | `Ctrl`+`K`, `Ctrl`+`K`                     | Toggle bookmark |
  |                                             | `Ctrl`+`Shift`+`K`<br />`Ctrl`+`Shift`+`L` | Clear bookmarks in document |
  </details>

  <details>
      <summary>Notes</summary>

  Bookmark Studio looks for the `.bookmarks.json` file in this order:

  1. Solution root  
  2. Repository root  
  3. `.vs` folder (default)  

  **Global** uses absolute document paths and is stored in `%USERPROFILE%`  
  **User** uses paths relative to the solution directory and is stored in the `.vs` folder  
  **Workspace** is for sharing with the team and is stored in the solution root or repository root  
  </details>

* [Customize Visual Studio Window Title [Erwin Mayer]](https://marketplace.visualstudio.com/items?itemName=mayerwin.RenameVisualStudioWindowTitle)
  * See [Custom Visual Studio Windows Title](#custom-visual-studio-windows-title) for configuration settings
* [JwtTools [Tim Heuer]](https://marketplace.visualstudio.com/items?itemName=TimHeuer.JwtTools)
* [Single-project MSIX Packaging Tools for VS 2022-26 [Microsoft]](https://marketplace.visualstudio.com/items?itemName=ProjectReunion.MicrosoftSingleProjectMSIXPackagingToolsDev17)
* [Solution Colors [Mads Kristensen]](https://marketplace.visualstudio.com/items?itemName=MadsKristensen.SolutionColors)
* [Spell Check My Code (VS2022 and Later) [EW Software]](https://marketplace.visualstudio.com/items?itemName=EWoodruff.VisualStudioSpellCheckerVS2022andLater)
* [Visual Studio Color Theme Designer 2022 [Microsoft DevLabs]](https://marketplace.visualstudio.com/items?itemName=idex.colorthemedesigner2022)
* [VsVim 2022 [Jared Parsons]](https://marketplace.visualstudio.com/items?itemName=JaredParMSFT.VsVim2022Preview)

#### Optional

* [Add New File (64-bit) [Mads Kristensen]](https://marketplace.visualstudio.com/items?itemName=MadsKristensen.AddNewFile64)
* [Extensibility Essentials 2022 [Mads Kristensen]](https://marketplace.visualstudio.com/items?itemName=MadsKristensen.ExtensibilityEssentials2022)
* [File Differ [Mads Kristensen]](https://marketplace.visualstudio.com/items?itemName=MadsKristensen.FileDiffer)
* [Rainbow Braces [Mads Kristensen]](https://marketplace.visualstudio.com/items?itemName=MadsKristensen.RainbowBraces)
* [SonarQube for Visual Studio [SonarSource]](https://marketplace.visualstudio.com/items?itemName=SonarSource.SonarLintforVisualStudio2022)
* [VSColorOutput64 [Mike Ward - Ann Arbor]](https://marketplace.visualstudio.com/items?itemName=MikeWard-AnnArbor.VSColorOutput64)

#### Themes

* [Marvel Theme [Guven Sezgin Kurt]](https://marketplace.visualstudio.com/items?itemName=GuvenSezginKurt.marvel239)
* [OLED Black Theme [Mads Kristensen]](https://marketplace.visualstudio.com/items?itemName=MadsKristensen.VantaBlack)
* [Vim VSTheme [Vital]](https://marketplace.visualstudio.com/items?itemName=Vital.VimVSTheme)
...
When checked, the using statements will be sorted with System first when sorting usings with **Edit → IntelliSense → Sort Usings** (Ctrl-K Ctrl-E).

* **Tools → Options → Text Editor → C# → Advanced**
  * **Using Directives** section
    * Check **Place 'System' directives first when sorting usings**
