...
#### Trailing Local Branches

- List all of the branches in the local repository that are behind the remote repository:

  ```powershell frame="none"
  git fetch
  git branch -vv | Select-String ': behind' | ForEach-Object { ($_.ToString().Substring(2)).Split(' ')[0] }
  ```
...
- List changes but exclude files matching patterns:

  ```shell frame="none"
  git diff branch-name -- . ':(exclude)**/*Feature.cs' ':(exclude)*Tests/**'
  ```

## Stash

- Stash all tracked, uncommitted changes (both staged and unstaged):

  ```shell frame="none"
  git stash push -m "WIP"
  ```

- Display all stashed entries on the stack:

  ```shell frame="none"
  git stash list
  ```

- Show the changes of the latest stash:

  ```shell frame="none"
  git stash show -p
  ```

- Reapply the most recent stash and keep it on the stash stack:

  ```shell frame="none"
  git stash apply
  ```

- Reapply the most recent stash and delete it from the stash stack:

  ```shell frame="none"
  git stash pop
  ```

- Delete a specific stash entry from the history stack without applying it:
