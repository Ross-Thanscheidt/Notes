...
##### Merge Conflict Resolution
  ...
  - Edit the files to choose which changes to keep,  
    removing the conflict marks (`<<<<<<<`, `=======`, `>>>>>>>`)

  - Mark each file as resolved (use `git add .` to mark all files as resolved at once):
  ...
  - Finalize the process by committing the changes to complete the merge:
