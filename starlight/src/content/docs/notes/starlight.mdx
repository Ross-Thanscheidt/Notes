...
- Add the npm dependencies needed for deployment:
...
- Add the `deploy` npm script to the `package.json` file:
...

    {
      ...
      "scripts": {
        ...
        "astro": "astro",
        "deploy": "astro build && shx touch dist/.nojekyll && gh-pages -d dist --dotfiles"
      },
      "dependencies": {
...
- Build the content to the `dist` directory and push to the remote `gh-pages` branch:
