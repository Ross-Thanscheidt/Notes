...
| endpoints.MapControllers()         | No    | Yes   |
...
Place this `GlobalSuppressions.cs` file with selected messages to suppress in the project folder  
(same folder as the `.csproj` file):
...
## Conditional Code by Target Frameworks
